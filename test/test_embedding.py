...
def test_embedding_forward():
    ...
    assert np.allclose(out.data, expected), (
        f"Forward mismatch: {out.data} vs {expected}"
    )
...
def test_embedding_backward():
    ...
    assert np.allclose(emb.weight.grad, expected_grad), (
        f"Gradient mismatch: {emb.weight.grad}"
    )
...
def test_embedding_repeated_indices():
    ...
    assert emb.weight.grad is not None, "Gradient is None"
    assert np.allclose(emb.weight.grad, expected_grad), (
        f"Repeated index gradient mismatch: {emb.weight.grad}"
    )
